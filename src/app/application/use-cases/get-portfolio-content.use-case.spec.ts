...
import { PortfolioContent, PortfolioContentPort } from '../ports/portfolio-content.port';
...
describe('GetPortfolioContentUseCase', () => {
  it('obtain the content through the port', async () => {
    ...
    const getPortfolioContent = vi.fn().mockResolvedValue(portfolioContent);

    const portfolioContentPort: PortfolioContentPort = {
      getPortfolioContent
    };

    const useCase = new GetPortfolioContentUseCase(portfolioContentPort);
